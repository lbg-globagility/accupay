...
namespace SampleBioK30.Utilities
{
    internal class DeviceManipulator
    {
        ...
        public ICollection<MachineInfo> GetLogData(ZkemClient objZkeeper, int machineNumber, DateTime? startDate = null, DateTime? endDate = null)
        {
            ...
            while (objZkeeper.SSR_GetGeneralLogData(machineNumber, out dwEnrollNumber1, out dwVerifyMode, out dwInOutMode, out dwYear, out dwMonth, out dwDay, out dwHour, out dwMinute, out dwSecond, ref dwWorkCode))
            {
                ...
                objInfo.IndRegID = dwEnrollNumber1;
                objInfo.DateTimeRecord = inputDate;
                objInfo.InOutMode = dwInOutMode;

                var dateTimeRecord = DateTime.Parse(objInfo.DateTimeRecord);
                if (dateTimeRecord >= startDate && dateTimeRecord <= endDate)
                {
