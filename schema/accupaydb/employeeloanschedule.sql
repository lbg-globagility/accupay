...
CREATE TABLE IF NOT EXISTS `employeeloanschedule` (
  ...
  `LoanPayPeriodLeft` decimal(10,2) NOT NULL,
  `Comments` varchar(2000) DEFAULT NULL,
  `BonusID` int(11) DEFAULT NULL,
  `LoanName` varchar(50) DEFAULT '',
  `LoanPayPeriodLeftForBonus` int(11) DEFAULT 0,
  `BonusPotentialPaymentForLoan` tinyint(1) DEFAULT 0,
  PRIMARY KEY (`RowID`),
  ...
  CONSTRAINT `FK_employee_bonus` FOREIGN KEY (`BonusID`) REFERENCES `employeebonus` (`RowID`) ON DELETE SET NULL,
  CONSTRAINT `FK_employeeloan_employee` FOREIGN KEY (`EmployeeID`) REFERENCES `employee` (`RowID`) ON DELETE CASCADE,
