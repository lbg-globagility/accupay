...
-- Dumping structure for view goldwingspayrolldb.v_sumthirteenthmonthpay
DROP VIEW IF EXISTS `v_sumthirteenthmonthpay`;
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `v_sumthirteenthmonthpay`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` VIEW `goldwingspayrolldb`.`v_sumthirteenthmonthpay` AS SELECT
tmp.RowID
,tmp.OrganizationID
,tmp.PaystubID
,tmp.Amount
