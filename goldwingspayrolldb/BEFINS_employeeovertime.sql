...
-- Dumping structure for trigger goldwingspayrolldb.BEFINS_employeeovertime
...
IF eshiftID IS NOT NULL THEN
	SET NEW.OTStartTime = (SELECT IF(TIME_FORMAT(sh.TimeTo,'%H:01') = TIME_FORMAT(NEW.OTStartTime,'%H:01'), ADDTIME(sh.TimeTo,'00:00:01'), NEW.OTStartTime) FROM shift sh WHERE sh.RowID=eshiftID);
	...
	SET NEW.OTEndTime = (SELECT IF(TIME_FORMAT(sh.TimeFrom,'%H:01') = TIME_FORMAT(NEW.OTEndTime,'%H:01'), SUBTIME(sh.TimeFrom,'00:00:01'), NEW.OTEndTime) FROM shift sh WHERE sh.RowID=eshiftID);
