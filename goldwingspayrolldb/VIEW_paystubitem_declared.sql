...
-- Dumping structure for procedure goldwingspayrolldb.VIEW_paystubitem_declared
...
,psa.TotalGrossSalary
,psa.TotalNetSalary
,IF(e.AgencyID IS NULL
    ,IF(BINARY `PAYMENT_SCHED_TO_CHAR`(d.WTaxDeductSched) = BINARY pp.`Half`, psa.TotalTaxableSalary, 0)
    ,IF(BINARY `PAYMENT_SCHED_TO_CHAR`(d.WTaxDeductSchedAgency) = BINARY pp.`Half`, psa.TotalTaxableSalary, 0)) AS TotalTaxableSalary
,psa.TotalEmpSSS
,psa.TotalEmpWithholdingTax
,psa.TotalCompSSS
,psa.TotalEmpPhilhealth
...
LEFT JOIN (
        ...
        LEFT JOIN (
                SELECT SUM(psi.PayAmount) `PayAmount`
                FROM paystubitem psi
                INNER JOIN product p
                    ON p.RowID=psi.ProductID
                    AND p.PartNo='Holiday pay'
                    AND psi.Undeclared=0
                INNER JOIN paystub ps
                    ON ps.EmployeeID=EmpRowID
                    AND ps.OrganizationID=OrganizID
                    AND ps.PayFromDate=pay_date_from
                    AND ps.PayToDate=pay_date_to
                    AND ps.RowID=psi.PayStubID
                INNER JOIN employee e
                    ON e.RowID=ps.EmployeeID
                    AND e.OrganizationID=ps.OrganizationID
                    AND e.EmployeeType IN ('Daily','Monthly')
            ) i
            ON i.PayAmount IS NULL
            OR i.PayAmount IS NOT NULL
        WHERE etea.EmployeeID=EmpRowID
            AND etea.OrganizationID=OrganizID
            AND etea.`Date` BETWEEN pay_date_from AND pay_date_to
    ) ete
    ON ete.eteRowID > 0
