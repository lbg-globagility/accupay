...
-- Dumping structure for procedure RECOMPUTE_thirteenthmonthpay
...
IF ispayperiodendofmonth IS NOT NULL THEN
    ...
    (
            ...
            INNER JOIN (
                ...
                ON esh.RowID = ete.EmployeeShiftID AND
                    esh.RestDay = FALSE
                WHERE ete.OrganizationID = OrganizID AND
                    `Date` BETWEEN month_firstdate AND last_date AND
                    esh.RestDay = 0
                GROUP BY ete.EmployeeID
            ) timeEntrySummary
            ON timeEntrySummary.EmployeeID = e.RowID
            INNER JOIN employeesalary es
            ON es.EmployeeID = e.RowID AND
                last_date BETWEEN es.EffectiveDateFrom AND IFNULL(es.EffectiveDateTo, last_date)
            WHERE e.PayFrequencyID = emppayfreqID AND
                e.EmployeeType = 'Daily' AND
                e.OrganizationID = OrganizID AND
                e.EmploymentStatus NOT IN ('Contractual', 'SERVICE CONTRACT')
        UNION
            SELECT
                (
                    timeEntrySummary.BasicAmount +
        ...
        UNION
            ...
            LEFT JOIN (
                ...
                WHERE
                    (HoursLateAmount + UndertimeHoursAmount + Absent) > 0 AND
                    OrganizationID = OrganizID AND
                    `Date` BETWEEN month_firstdate AND last_date
                GROUP BY EmployeeID
            ) et
            ON et.EmployeeID = e.RowID
            WHERE e.PayFrequencyID = emppayfreqID AND
                e.OrganizationID = OrganizID AND
                e.EmployeeType IN ('Monthly', 'Fixed')
    ) ii
    INNER JOIN paystub ps
    ON ps.OrganizationID = OrganizID AND
