...
-- Dumping structure for procedure goldwingspayrolldb.paystub_payslip
...
FROM (
    ...
    INNER JOIN employeesalary es
        ON es.EmployeeID=ps.EmployeeID
        AND es.OrganizationID=ps.OrganizationID
        AND (es.EffectiveDateFrom >= ps.PayFromDate OR IFNULL(es.EffectiveDateTo,ps.PayToDate) >= ps.PayFromDate)
        AND (es.EffectiveDateFrom <= ps.PayToDate OR IFNULL(es.EffectiveDateTo,ps.PayToDate) <= ps.PayToDate)
    LEFT JOIN (
            SELECT
                RowID,
                EmployeeID,
            ...
            FROM (
                    ...
                    WHERE OrganizationID=OrganizID
                        AND IsActualFlag=0
                        AND `Date` BETWEEN paydate_from AND paydat_to
                UNION
                    SELECT
                        RowID,
                        OrganizationID,
        ...
        AND ete.EmployeeID=ps.EmployeeID
    LEFT JOIN (
            SELECT
            ...
            INNER JOIN product
                ...
                AND product.OrganizationID = paystubitem.OrganizationID
                AND product.`Category` = 'Allowance Type'
    ...
    LEFT JOIN (
            ...
            INNER JOIN paystub
                ON paystub.PayPeriodID = scheduledloansperpayperiod.PayPeriodID
                AND paystub.EmployeeID = scheduledloansperpayperiod.EmployeeID
            INNER JOIN paystubitem
                ON paystubitem.PayStubID = paystub.RowID
                AND paystubitem.ProductID = employeeloanschedule.LoanTypeID
            INNER JOIN product
                ON product.RowID = paystubitem.ProductID
    ...
    LEFT JOIN (
            ...
            INNER JOIN product p
                ON p.RowID=psi.ProductID
                AND p.OrganizationID=psi.OrganizationID
                AND p.PartNo='Holiday pay'
            WHERE psi.Undeclared=IsActualFlag
    ...
    LEFT JOIN (
            ...
            FROM (
                    ...
                    WHERE IsActualFlag = 0
                        AND paystubadjustment.OrganizationID = OrganizID
                        AND paystubadjustment.PayAmount != 0
                UNION
                    SELECT
                    ...
                    WHERE IsActualFlag = 1
                        AND paystubadjustmentactual.OrganizationID = OrganizID
                        AND paystubadjustmentactual.PayAmount != 0
            ) adjustment
