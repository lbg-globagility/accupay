...
-- Dumping structure for view goldwingspayrolldb.vw_disciplinarymemouserdetails
DROP VIEW IF EXISTS `vw_disciplinarymemouserdetails`;
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `vw_disciplinarymemouserdetails`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` VIEW `goldwingspayrolldb`.`vw_disciplinarymemouserdetails` AS SELECT 
	U.RowID AS 'UserID',
	U.FirstName, 
	U.MiddleName, 
	U.LastName,
