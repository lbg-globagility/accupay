...
-- Dumping structure for procedure goldwingspayrolldb.GET_employee_allowanceofthisperiod
...
ELSEIF AllowanceFrequenzy = 'Daily version 1' THEN
    ...
    SELECT et.EmployeeID,
           ...
           IF(LOCATE('Regular Holi',pr.PayType) > 0 AND LOCATE('cola',p.PartNo) > 0 AND et.TotalDayPay > 0,
           ea.AllowanceAmount,
           ((et.RegularHoursWorked / @timediffcount) * ea.AllowanceAmount)) AS TotalAllowanceAmount,
           ea.ProductID,
           et.`Date`
    FROM employeetimeentry et
    INNER JOIN employee e
            ON e.OrganizationID=OrganizID
           AND e.RowID=et.EmployeeID
    ...
    INNER JOIN employeeallowance ea
            ON ea.AllowanceFrequency=AllowanceFrequenzy
           AND ea.TaxableFlag=IsTaxable
    ...
    WHERE et.OrganizationID=OrganizID
      AND et.`Date` BETWEEN DatePayFrom AND DatePayTo;
...
ELSEIF AllowanceFrequenzy = 'Daily' THEN
    ...
    WHERE i.TaxableFlag=IsTaxable
        AND i.OrganizationID=OrganizID
        AND i.`Date` BETWEEN DatePayFrom AND DatePayTo
        AND i.`Fixed`=0
UNION
    SELECT et.RowID,
           et.EmployeeID,
...
UNION
    ...
    INNER JOIN employeeallowance ea
            ON ea.AllowanceFrequency=AllowanceFrequenzy
           AND IF(IsTaxable='1', (ea.TaxableFlag=IsTaxable), FALSE)
           AND ea.EmployeeID=e.RowID
           AND ea.OrganizationID=OrganizID
           AND et.`Date` BETWEEN ea.EffectiveStartDate AND ea.EffectiveEndDate
    INNER JOIN product p
