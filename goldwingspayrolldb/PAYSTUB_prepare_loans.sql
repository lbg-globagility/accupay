...
-- Dumping structure for procedure goldwingspayrolldb.PAYSTUB_prepare_loans
DROP PROCEDURE IF EXISTS `PAYSTUB_prepare_loans`;
DELIMITER //
CREATE DEFINER=`root`@`127.0.0.1` PROCEDURE `PAYSTUB_prepare_loans`(IN `OrganizID` INT, IN `Date_From` DATE, IN `Date_To` DATE, IN `ppRowID` INT)
    DETERMINISTIC
BEGIN
...
SELECT pp.`Half`,pp.TotalGrossSalary FROM payperiod pp INNER JOIN payperiod p2 ON p2.RowID=ppRowID AND pp.TotalGrossSalary=p2.TotalGrossSalary WHERE pp.OrganizationID=OrganizID AND pp.PayFromDate=Date_From AND pp.PayToDate=Date_To LIMIT 1 INTO IsFirstHalfOfMonth,payfreqID;
...
IF IsFirstHalfOfMonth = '1' THEN
   ...
   INNER JOIN employee e ON e.RowID=els.EmployeeID AND e.OrganizationID=els.OrganizationID AND e.PayFrequencyID=payfreqID
