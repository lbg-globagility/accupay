...
-- Dumping structure for trigger goldwingspayrolldb.AFTINS_paystubitem_then_employeeloanhistory
DROP TRIGGER IF EXISTS `AFTINS_paystubitem_then_employeeloanhistory`;
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `AFTINS_paystubitem_then_employeeloanhistory` AFTER INSERT ON `paystubitem` FOR EACH ROW BEGIN
...
DECLARE is_LastDateOfMonth TEXT;
...
SELECT PayPeriodID,EmployeeID,PayToDate,PayFromDate FROM paystub WHERE RowID=NEW.PayStubID INTO paypID,empID,pay_todate,pay_fromdate;
...
SELECT (CategoryID = categoryRowID) FROM product WHERE RowID=NEW.ProductID INTO isItemLoan;
	...
	IF isItemLoan = '1' THEN
		...
		(
			...
			,pay_todate
			,NEW.PayAmount
			,(SELECT PartNo FROM product WHERE RowID=NEW.ProductID)
		);
...
IF item_categName = 'Loan Type' THEN
