...
END IF;
	IF IFNULL(Branch_Code,'') != '' THEN INSERT INTO branch(OrganizationID,Created,CreatedBy,BranchCode) VALUES (etentd_OrganizationID,CURRENT_TIMESTAMP(),etentd_CreatedBy,Branch_Code) ON DUPLICATE KEY UPDATE LastUpd=CURRENT_TIMESTAMP(); END IF;
	SELECT RowID FROM branch WHERE OrganizationID=etentd_OrganizationID AND BranchCode=IFNULL(Branch_Code,'') LIMIT 1 INTO branch_rowid;
...
IF EditAsUnique = '1'
	...
	UPDATE
		...
		,TimeStampOut=TIMESTAMP(ADDDATE(DateTimeLogOut, INTERVAL 0 SECOND));SELECT @@Identity AS id INTO etentdID;
