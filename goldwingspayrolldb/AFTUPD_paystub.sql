...
-- WHERE p.PartNo = 'Holiday pay'
--     AND p.`Category` = 'Miscellaneous'
...
-- SELECT RowID
-- FROM product
-- WHERE PartNo = 'Night differential'
--     AND OrganizationID = NEW.OrganizationID
-- INTO product_rowid;
...
-- SELECT SUM(NightDiffHoursAmount)
-- FROM employeetimeentry
-- WHERE EmployeeID=NEW.EmployeeID
--     AND OrganizationID=NEW.OrganizationID
--     AND `Date` BETWEEN NEW.PayFromDate AND NEW.PayToDate
...
--     ON c.CategoryName = 'Allowance Type'
...
-- SELECT GROUP_CONCAT(DISTINCT(ea.ProductID))
...
-- FROM (
--     SELECT
--         DISTINCT(i.etRowID) AS et_RowID,
--         i.ProductID,i.TotalAllowanceAmt
...
--                 ON e.OrganizationID=NEW.OrganizationID
--                 AND e.RowID=NEW.EmployeeID
...
--             INNER JOIN employeeallowance ea
--                 ON ea.AllowanceFrequency='Monthly'
--                 AND ea.EmployeeID=e.RowID
--                 AND ea.OrganizationID=NEW.OrganizationID
--                 AND et.`Date` BETWEEN ea.EffectiveStartDate AND ea.EffectiveEndDate
--             INNER JOIN product p
...
-- FROM product p
-- INNER JOIN category c
--     ON c.CategoryName = 'Loan Type'
--     AND c.OrganizationID = NEW.OrganizationID
--     AND c.RowID = p.CategoryID
-- WHERE p.OrganizationID = NEW.OrganizationID
-- INTO loantype_IDs;

-- SELECT GROUP_CONCAT(DISTINCT(els.LoanTypeID))
...
-- SELECT INS_audittrail_RETRowID(
--     NEW.LastUpdBy,
--     NEW.LastUpdBy,
--     NEW.OrganizationID,
--     viewRowID,
