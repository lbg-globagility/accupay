...
Public Class TimeAttendanceAnalyzer
    ...
    Public Function FourHoursRuleBasedOnEmployeeShiftFinalTest(logs As IList(Of TimeAttendanceLog), employeeShifts As IList(Of ShiftSchedule), employeeSpecificCounts As IList(Of TimeEmployeeNo)) As IList(Of TimeLogInOut)

        Dim list = New List(Of TimeLogInOut)
        Dim listIn = New List(Of TimeLogsTempIn)
        Dim listOut = New List(Of TimeLogsTempOut)
        For Each employeeSpecificCount In employeeSpecificCounts
            For Each employeeShift In employeeShifts
                ...
                For Each log In logs.Where(Function(x) x.EmployeeNo = employeeSpecificCount.EmployeeID)
