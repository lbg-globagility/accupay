...
Public Class BankFileSimpleExcelForm
    ...
    Private Async Sub btnExport_Click(sender As Object, e As EventArgs) Handles btnExport.Click
        ...
        Using excel = New ExcelPackage(newFile:=saveFileDialogHelperOutPut.FileInfo)
            ...
            If defaultWorksheet Is Nothing Then defaultWorksheet = excel.Workbook.Worksheets.Add("eCredit")

            Dim rowIndex = 2
            For Each model In models
                defaultWorksheet.Cells(rowIndex, 1).Value = "DTL"
                defaultWorksheet.Cells(rowIndex, 2).Value = model.AccountNumberSimpleExcelCompliant
                defaultWorksheet.Cells(rowIndex, 3).Value = model.Amount.ToString("N")
                defaultWorksheet.Cells(rowIndex, 4).Value = model.FullNameBeginningWithLastName

                rowIndex += 1
            Next

            defaultWorksheet.Cells(rowIndex, 1).Value = "TLR"
            defaultWorksheet.Cells(rowIndex, 2).Value = If(models?.Count(), 0)
